...
class RedisQueue(object):
    ...
    """
    def __init__(self, name, addr, port, dbnum):
        self.key = name
        self.rq = redis.Redis(addr, port, dbnum)
    ...
    def put_id(self, id: int):
        self.rq.lpush(self.key, str({"id":id}))
    ...
    #def get_data_id(self, isBlocking=False, timeout=None) -> int:
    #    element = self.get(isBlocking=isBlocking, timeout=timeout)
    #    if element is not None:
    #        msg_json = json.loads(msg.decode('utf-8'))
    #        return int(msg_json['id'])
    #    raise ValueError
